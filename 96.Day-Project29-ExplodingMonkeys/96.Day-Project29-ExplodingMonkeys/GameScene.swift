...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if firstNode.name == "banana" && secondNode.name == "player1" {
            if viewController!.player2Win < 2 {
                destroy(player: player1)
                viewController?.player2Win += 1
            } else {
                destroy(player: player1)
        ...
        if firstNode.name == "banana" && secondNode.name == "player2" {
            if viewController!.player1Win < 2 {
                destroy(player: player2)
                viewController?.player1Win += 1
            } else {
                destroy(player: player2)
    ...
    func launch(angle: Int, velocity: Int) {
        // how hard broke banana
        let speed = Double(velocity) / 10.0
        
        //attached angle in radians
        let radiands = deg2rad(degrees: angle)
        ...
        if currentPlayer == 1 {
            ...
            let impulse = CGVector(dx: cos(radiands) * speed,
                                   dy: sin(radiands) * speed)
    ...
    func destroy(player: SKSpriteNode) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.viewController.gameOverLabel.isHidden = true
            }
            
            if self.viewController.player1Win == 3 || self.viewController.player2Win == 3 {
                self.viewController.player1Win = 0
                self.viewController.player2Win = 0
            }
