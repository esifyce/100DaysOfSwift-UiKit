...
class BuildingNode: SKSpriteNode {
    ...
    func hit(at point: CGPoint) {
        let converedPoint = CGPoint(x: point.x + size.width / 2, y: abs(point.y - (size.height / 2)))
        
        let renderer = UIGraphicsImageRenderer(size: size)
        let img = renderer.image { ctx in
            currentImage.draw(at: .zero)
            
            ctx.cgContext.addEllipse(in: CGRect(x: converedPoint.x - 32,
                                                y: converedPoint.y - 32,
                                                width: 64,
                                                height: 64))
