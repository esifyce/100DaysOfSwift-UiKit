...
class SelectionViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let saveItem = defaults.object(forKey: "saveItem") as? [String] {
            saveItems = saveItem
        }
        ...
        if saveItems.isEmpty {
            for i in 0...items.count - 1 {
                ...
                if let jpegData = original.jpegData(compressionQuality: 0.8) {
                    ...
                    saveItems.append(pathDD.path)
                }
            }
            let defaults = UserDefaults.standard
            defaults.set(saveItems, forKey: "saveItem")
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let rounded = renderer.image { ctx in
            ...
            if let original = UIImage(contentsOfFile: saveItems[indexPath.row % saveItems.count]) {
